...
function getConnectedOrganizerWallet(
  ...
  if (
    ...
    throw new Error(
      "Only the organizer wallet can use these controls.",
    );
  }
...
async function requestAttendees(
  ...
  const data =
    (await response.json()) as AttendeesResponse;
...
async function requestExport(
  ...
  const response =
    await fetch(
      `/api/events/${encodeURIComponent(
        input.eventId,
      )}/attendees/export?format=${input.format}`,
      {
        method: "POST",
        headers: {
          "Content-Type":
...
<head>
  ...
  <style>
    * { box-sizing: border-box; }
    ...
    th, td { border: 1px solid #d7d7d7; padding: 7px; text-align: left; vertical-align: top; }
...
<body>
  ...
  <div class="meta">
    ...
    Event type: ${escapeHtml(
      event?.eventTypeLabel ??
        "Not available",
    )}<br />

    Organizer: ${escapeHtml(
      event?.organizer ?? "",
    )}<br />

    Upfront / Deposit: ${escapeHtml(
      event?.deposit?.formatted ??
        "0",
    )} USDC<br />

    Total price: ${escapeHtml(
      event?.totalPrice?.formatted ??
        "0",
    )} USDC<br />

    Total attendees: ${escapeHtml(
      data.attendeeCount ??
        String(
          attendees.length,
        ),
  ...
  <table>
    ...
    <tbody>
      ${
        attendeeRows ||
        '<tr><td colspan="6">No attendees found for this event.</td></tr>'
      }
    </tbody>
  </table>
...
export default function OrganizerAttendancePanel({
  ...
  async function handleConfirmAttendance(
    ...
    try {
      ...
      setMessage(
        isPaidEvent
          ? "Preparing paid-event attendance confirmation..."
      ...
      if (isPaidEvent) {
        setMessage(
          `Enter your Circle PIN to confirm ${shortenAddress(
            attendee,
          )}. Their remaining payment will become due for three days.`,
        );
      } else if (
        hasDeposit
      ) {
        setMessage(
          `Enter your Circle PIN to confirm ${shortenAddress(
            attendee,
          )}. Their ${currentDeposit} USDC refundable deposit will be returned.`,
        );
      } else {
        setMessage(
          `Enter your Circle PIN to confirm ${shortenAddress(
            attendee,
          )}. No USDC transfer is required.`,
        );
      }
      ...
      if (isPaidEvent) {
        setMessage(
          `Attendance confirmed for ${shortenAddress(
            attendee,
          )}. The remaining balance is now due within three days.`,
        );
      } else if (
        hasDeposit
      ) {
        setMessage(
          `Attendance confirmed. ${currentDeposit} USDC was returned to ${shortenAddress(
            attendee,
          )}.`,
        );
      } else {
  ...
  async function handleSettleNoShow(
    attendee: string,
  ) {
    if (busyAttendee) {
      return;
    ...
    try {
      ...
      if (!hasDeposit) {
        setMessage(
          `Enter your Circle PIN to mark ${shortenAddress(
            attendee,
          )} as a no-show. No USDC will be transferred.`,
        );
      } else if (
        isPaidEvent
      ) {
        setMessage(
          `Enter your Circle PIN to mark ${shortenAddress(
            attendee,
          )} as a no-show. Their ${currentDeposit} USDC upfront payment will be transferred to the organizer.`,
        );
      } else {
        setMessage(
          `Enter your Circle PIN to mark ${shortenAddress(
            attendee,
          )} as a no-show. Their ${currentDeposit} USDC deposit will be transferred to the organizer.`,
        );
      }
      ...
      setMessage(
        "Waiting for no-show settlement on Arc Testnet...",
      );
      ...
      if (!hasDeposit) {
        ...
        setMessage(
          `${shortenAddress(
            attendee,
          )} was marked as a no-show. ${currentDeposit} USDC was transferred to the organizer.`,
        );
      }
      ...
      console.error(
        "ShowUp no-show settlement failed:",
        settleError,
      );
  ...
  async function handleMarkPaymentDefault(
    attendee: string,
  ) {
    if (busyAttendee) {
      return;
    ...
    try {
      ...
      setMessage(
        "Preparing the payment-default transaction...",
      );

      const response =
        await fetch(
          "/api/circle/events/mark-payment-default",
          {
            method: "POST",
            headers: {
              "Content-Type":
      ...
      setMessage(
        `Enter your Circle PIN to mark ${shortenAddress(
          attendee,
        )} as payment defaulted. Their upfront payment will be transferred to the organizer.`,
      );
      ...
      setMessage(
        "Waiting for payment default confirmation on Arc Testnet...",
      );
      ...
      if (!confirmed) {
        throw new Error(
          "The transaction was submitted but payment default has not been confirmed yet. Refresh shortly.",
        );
      }
      ...
      setMessage(
        `${shortenAddress(
          attendee,
        )} was marked as payment defaulted. The ${currentDeposit} USDC upfront payment was transferred to the organizer.`,
      );
  ...
  async function handleCsvExport() {
    ...
    setMessage(
      "Preparing the complete attendee CSV...",
    );
    ...
    try {
      ...
      const filename =
        filenameMatch?.[1] ??
        `showup-event-${eventId}-attendees.csv`;
  ...
  async function handlePrintReport() {
    ...
    if (!printWindow) {
      setError(
        "Allow pop-ups to create the printable attendee report.",
      );
    ...
    setMessage(
      "Preparing the complete printable report...",
    );
    ...
    try {
      ...
      const data =
        (await response.json()) as PrintExportResponse;
      ...
      setMessage(
        "The complete report is ready for Print / Save as PDF.",
      );
    } catch (
      printError
    ) {
  ...
  return (
    <section className="mt-6 rounded-[26px] border border-[#74f2c2]/20 bg-[#74f2c2]/[0.055] p-5">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="mt-2 text-sm leading-6 text-white/45">
            {isPaidEvent
              ? `Confirm attendance to open the remaining-payment window. Upfront: ${currentDeposit} USDC. Total price: ${totalPrice} USDC.`
              : hasDeposit
                ? `Confirm attendance to return each attendee's ${currentDeposit} USDC refundable commitment deposit.`
                : "Confirm attendance for free reservations. No USDC refund is required."}
          </p>
        </div>
      ...
      {isPaidEvent ? (
        <div className="mt-3 rounded-2xl border border-amber-300/15 bg-amber-300/[0.055] px-4 py-3 text-xs leading-5 text-amber-100/70">
          Paid attendees receive three days after attendance confirmation to pay the remaining balance. Payment default can only be marked after that individual deadline.
        </div>
      ) : null}
      ...
      <div className="mt-4 flex flex-wrap gap-3">
        ...
        <label className="w-full sm:w-[220px] sm:flex-none">
          <span className="sr-only">
          ...
          >
            ...
            <option value="payment-defaulted">
              Payment defaulted
            </option>
            ...
            <option value="fallback-refunded">
              Fallback refunded
            </option>
      ...
      <div className="mt-3 flex flex-wrap items-center justify-between gap-2 text-xs text-white/35">
        <p>
          Showing {showingFrom}-
          {showingTo} of{" "}
          {filteredCount} matching attendees
        </p>

        <p>
          Total attendee wallets:{" "}
          {attendeeCount}
        </p>
      </div>
                  ...
                  <div className="min-w-0">
                    ...
                    {attendee.status ===
                      ...
                      <p className="mt-1 text-xs leading-5 text-emerald-100/65">
                        The remaining balance was paid and the reservation is complete.
                      </p>
                    ) : null}
                    ...
                    {attendee.status ===
                      ...
                      <p className="mt-1 text-xs leading-5 text-orange-100/65">
                        The remaining balance was not paid before the deadline.
                      </p>
                    ) : null}
                  </div>
                    ...
                    <div className="grid w-full grid-cols-1 gap-2">
                      ...
                      >
                        ...
                        busyAction ===
                          ...
                          ? "Confirming..."
                          : isPaidEvent
                            ? "Confirm attendance and open payment"
                            : "Confirm attendance"}
                      </button>
                  ...
                  7 ? (
                    <div className="w-full">
                      <button
                        ...
                        className="w-full rounded-xl border border-orange-300/25 bg-orange-300/10 px-4 py-2.5 text-xs font-semibold text-orange-100 transition hover:border-orange-300/40 hover:bg-orange-300/15 disabled:cursor-not-allowed disabled:opacity-40"
                      >
                        {processing &&
                        busyAction ===
                          "payment-default"
                          ? "Marking default..."
                          : paymentDeadlinePassed
                            ? "Mark payment default"
                            : "Waiting for payment deadline"}
                      </button>
                    </div>
                  ) : null}
                </div>
